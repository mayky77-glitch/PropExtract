...
def test_hosted_non_windows_is_a_typed_safe_negative_path(tmp_path: Path) -> None:
    request = NativeInsertRequest("op", "owner", "pair", tmp_path / "control.xlsx", tmp_path / "candidate.xlsx", 6, tmp_path / "lease.json", tmp_path / "ack.json")
...
def test_native_request_carries_pair_and_lease_paths(tmp_path: Path) -> None:
    request = NativeInsertRequest("op", "owner", "pair", tmp_path / "control.xlsx", tmp_path / "candidate.xlsx", 10, tmp_path / "lease.json", tmp_path / "ack.json")
