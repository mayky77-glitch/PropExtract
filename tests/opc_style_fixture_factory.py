...
def package(destination: Path, *, style_xml: bytes | None = None, styles_relationship: str | None = None, style_name: str = "xl/styles.xml", style_override: str | None = None, sheet_one: bytes | None = None, sheet_two: bytes | None = None, extra_members: tuple[tuple[str, bytes], ...] = ()) -> Path:
    ...
    workbook_rels = relationship("one", f"{OFFICE_REL_NS}/worksheet", "worksheets/first.xml") + relationship("two", f"{OFFICE_REL_NS}/worksheet", "worksheets/second.xml")
    ...
    content = (f'<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/first.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/worksheets/second.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>{override}</Types>').encode()
    ...
    members=[("[Content_Types].xml",content),("_rels/.rels",f'<Relationships xmlns="{REL_NS}">{relationship("root",f"{OFFICE_REL_NS}/officeDocument","xl/workbook.xml")}</Relationships>'.encode()),("xl/workbook.xml",workbook),("xl/_rels/workbook.xml.rels",f'<Relationships xmlns="{REL_NS}">{workbook_rels}</Relationships>'.encode()),("xl/worksheets/first.xml",sheet_one or worksheet('<row r="6"><c r="A6" s="0"><v>1</v></c></row><row r="10"><c r="B10"><v>2</v></c></row><row r="104"><c r="C104" s="1"><v>3</v></c></row>')),("xl/worksheets/second.xml",sheet_two or worksheet('<row r="6"><c r="A6"><v>1</v></c></row>')),(style_name,style_xml or default)] + list(extra_members)
