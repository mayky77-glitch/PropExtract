...
def test_mismatch_or_pid_reuse_never_kills(inspector: Inspector) -> None:
    with pytest.raises(ExcelProcessCleanupError, match="excel_cleanup_identity_mismatch"):
        cleanup_excel_process(_lease(), prelaunch_excel_pids=frozenset(), inspector=inspector)
    assert inspector.kills == []


def test_preexisting_inaccessible_and_missing_are_distinguished() -> None:
    ...
    missing = Inspector(identity=ProcessMissingError())
    cleanup_excel_process(_lease(), prelaunch_excel_pids=frozenset(), inspector=missing)
    assert preexisting.kills == inaccessible.kills == missing.kills == []
