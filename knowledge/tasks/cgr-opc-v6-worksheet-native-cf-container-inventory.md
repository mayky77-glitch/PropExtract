...
- P6 remediation replaced raw duplicate-attribute scanning with one namespace-aware expat event pipeline. Expanded-QName duplicate handling now follows encoding/XML then x14 precedence and does not inspect unused DTD entity text. Focused command passed `322`; full suite passed `1074` with the same existing OpenPyXL warning.
