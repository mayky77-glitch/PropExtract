...
Implemented from frozen base `c5ea7cab467288d15b3ec0075d6a87a424633ca0` with accepted X1 dependency `ea280eeb2101c87213bc538d3b697fc0ea6a982e`. The envelope reader uses the shared accepted X1 parse/ownership pipeline and publishes only after all worksheets pass X1 and X2 validation. P6 follow-up fixes priority lexical handling for XML whitespace and signed/zero-padded positive values, reports numeric duplicate priorities, and defers container structure errors until earlier rule semantics validate. Exact acceptance subset: 551 passed. Full suite: 1303 passed, with one existing openpyxl unknown-extension warning. Required `compileall` and `git diff --check` passed.
