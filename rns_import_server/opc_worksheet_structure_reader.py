...
from .opc_worksheet_cell_reader import WorksheetCells, read_worksheet_cell_semantics
...
def _row_unsigned(value: str, part: str, field: str) -> int:
    lexical = value.strip()
    if len(lexical) > 64 or _ROW_UNSIGNED.fullmatch(lexical) is None:
        _fail("invalid-row-property", part, field, value)
    numeric = int(lexical)
    if numeric > _MAX_UINT:
        _fail("invalid-row-property", part, field, value)
    return numeric
...
def _xml(payload: bytes, part: CanonicalPartURI) -> ET.Element:
    """Parse only XML already accepted by the cell-reader boundary."""
    try:
        root = ET.fromstring(payload)
    except (ET.ParseError, LookupError, UnicodeError, ValueError) as error:
        _fail("worksheet-xml-boundary-mismatch", part.value, "xml", type(error).__name__)
    if root.tag != _WORKSHEET:
        _fail("worksheet-xml-boundary-mismatch", part.value, "root", str(root.tag))
...
def _row(element: ET.Element, part: str, previous: int) -> WorksheetRowProperties:
    ...
    if "ht" in element.attrib:
        # The cell reader is the single owner of lexical validation. Its
        # accepted scientific/whitespace forms are intentionally preserved.
        height = float(element.attrib["ht"].strip())
    style = _row_unsigned(element.attrib["s"], part, "s") if "s" in element.attrib else None
    outline = _row_unsigned(element.attrib["outlineLevel"], part, "outlineLevel") if "outlineLevel" in element.attrib else None
...
def _structure(root: ET.Element, part: CanonicalPartURI, cells: WorksheetCells) -> tuple[A1Range | None, tuple[WorksheetRowProperties, ...], tuple[A1Range, ...], WorksheetAutoFilter | None]:
    ...
    expected_tags = {
        "dimension": _DIMENSION, "sheetData": _SHEET_DATA, "row": _ROW,
        "autoFilter": _AUTO_FILTER, "mergeCells": _MERGE_CELLS, "mergeCell": _MERGE_CELL,
    }
    ...
    if merge_container is not None:
        ...
        if "count" not in merge_container.attrib:
