...
_V7_REFRESH_RECEIPT_COLUMNS = frozenset({
...
class RegistryStorage:
    ...
    def _create_schema(self, *, seed_revision: str) -> None:
        ...
        self.connection.executescript(
            """
                ...
                CREATE TABLE workbook_authority_refresh_receipts (
                    operation_id TEXT PRIMARY KEY REFERENCES workbook_operation_journal(operation_id) ON DELETE RESTRICT,
                    action_id TEXT NOT NULL UNIQUE REFERENCES new_row_pending_actions(action_id) ON DELETE RESTRICT,
                    receipt_version INTEGER NOT NULL CHECK (receipt_version = 1),
                    manifest_version TEXT NOT NULL CHECK (manifest_version = 'group-row-manifest-v3'),
                    mutation_mode TEXT NOT NULL CHECK (mutation_mode IN ('blank_fill', 'middle_insert')),
                    target_row INTEGER NOT NULL CHECK (target_row >= 2),
                    pre_hash TEXT NOT NULL CHECK (length(pre_hash) = 64 AND pre_hash NOT GLOB '*[^0-9a-f]*'),
                    post_hash TEXT NOT NULL CHECK (length(post_hash) = 64 AND post_hash NOT GLOB '*[^0-9a-f]*'),
                    ...
                    predecessor_payload TEXT NOT NULL,
                    predecessor_digest TEXT NOT NULL CHECK (length(predecessor_digest) = 64 AND predecessor_digest NOT GLOB '*[^0-9a-f]*'),
                    successor_payload TEXT NOT NULL,
                    successor_digest TEXT NOT NULL CHECK (length(successor_digest) = 64 AND successor_digest NOT GLOB '*[^0-9a-f]*'),
    ...
    def _migrate(self, version: int) -> None:
        ...
        with self.transaction() as connection:
            ...
            # v7 deliberately starts as an empty receipt ledger: no legacy
            # authority/action/journal evidence is inferred or backfilled.
            connection.execute(
                """CREATE TABLE IF NOT EXISTS workbook_authority_refresh_receipts (
                    operation_id TEXT PRIMARY KEY REFERENCES workbook_operation_journal(operation_id) ON DELETE RESTRICT,
                    action_id TEXT NOT NULL UNIQUE REFERENCES new_row_pending_actions(action_id) ON DELETE RESTRICT,
                    receipt_version INTEGER NOT NULL CHECK (receipt_version = 1),
                    manifest_version TEXT NOT NULL CHECK (manifest_version = 'group-row-manifest-v3'),
