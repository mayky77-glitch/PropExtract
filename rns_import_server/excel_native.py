...
class _WindowsInspector:
    ...
    def _powershell(expression: str) -> dict[str, Any]:
        ...
        if completed.returncode:
            if "cannot find a process" in completed.stderr.lower(): raise ProcessMissingError()
    ...
    def process_identity(self, pid: int) -> ProcessIdentity:
        value = self._powershell("$p=Get-Process -Id %d -ErrorAction Stop; @{pid=[int]$p.Id;image=([string]$p.ProcessName+'.exe');started_at=$p.StartTime.ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')}|ConvertTo-Json -Compress" % pid)
        return ProcessIdentity(value["pid"], value["image"], value["started_at"])
...
def _private_logs(request: NativeInsertRequest) -> tuple[Path, Path]:
    try:
        ...
        raise NativeExcelError("excel_private_log_unavailable", stage="launch", cause=error) from error
...
def _audit_ack(request: NativeInsertRequest) -> None:
    try:
        ...
        raise NativeExcelError("excel_lease_ack_failed", stage="ack", cause=error) from error
...
def run_native_insert(request: NativeInsertRequest, script: Path, journal: _Journal, timeout: float = 120.0) -> dict[str, Any]:
    ...
    if not isinstance(request.mutation_mode, str) or request.mutation_mode not in {"middle_insert", "blank_fill"}: raise NativeExcelError("native_mutation_mode_invalid", stage="pre_open")
    if request.insertion_row < 1: raise NativeExcelError("native_insert_row_invalid", stage="pre_open")
    ...
    except Exception as error: raise NativeExcelError("excel_private_log_unavailable", stage="launch", cause=error) from error
    process: subprocess.Popen[bytes] | None = None; lease: ExcelProcessLease | None = None; primary: NativeExcelError | None = None
    ...
    try:
        ...
        try:
            ...
            raise NativeExcelError("excel_private_log_unavailable", stage="adapter", cause=error, durable_phase="native") from error
        ...
        primary = NativeExcelError("excel_adapter_launch_failed", stage="adapter", cause=error); raise primary from error
    finally:
