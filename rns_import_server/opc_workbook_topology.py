...
from .opc_part_uri import CanonicalPartURI
...
def _read_required_xml(package_path: os.PathLike[str] | str, workbook_part: CanonicalPartURI) -> tuple[bytes, bytes]:
    try:
        path = os.fspath(package_path)
        with ZipFile(path) as package:
            return package.read(workbook_part.value), package.read("[Content_Types].xml")
...
def _require_main_workbook_content_type(payload: bytes, workbook_part: CanonicalPartURI) -> None:
    ...
    values = tuple(child.attrib.get("ContentType", "") for child in root if child.tag == _OVERRIDE_TAG and child.attrib.get("PartName") == expected_part_name)
    if not values:
...
def _descriptor(
    ...
    names: set[str], sheet_ids: set[int], relationship_ids: set[str],
) -> WorksheetDescriptor:
    ...
    names.add(name); sheet_ids.add(sheet_id); relationship_ids.add(relationship_id)
...
def read_workbook_topology(package_path: os.PathLike[str] | str) -> WorkbookTopology:
    ...
    try:
        graph = build_opc_package_graph(package_path)
    ...
    workbook_xml, content_types_xml = _read_required_xml(package_path, workbook_part)
    ...
    names: set[str] = set(); sheet_ids: set[int] = set(); relationship_ids: set[str] = set()
    ...
    for element in sheets:
        ...
        descriptors.append(_descriptor(element, graph.relationships, workbook_part, names, sheet_ids, relationship_ids))
